...
bash "$SCRIPT_DIR/../9.cc-deploy/lifecycle_chaincodes.sh"
echo "applying api from 10.api"
kubectl apply -f "$SCRIPT_DIR/../10.api/k8/configmap.yaml"
kubectl apply -f "$SCRIPT_DIR/../10.api/k8/couchdb.yaml"
kubectl apply -f "$SCRIPT_DIR/../10.api/k8/api.yaml"
echo "applying bgp route from 10.api"
bash "$SCRIPT_DIR/../10.api/bgp-k8s/gobgp-k8s.yaml"
