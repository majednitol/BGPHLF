...
for ns in r1 r2 r3; do
  ...
  ip netns exec $ns env \
    VTYSH_API_SOCKET=/var/run/frr/$ns/vtysh.sock \
    /usr/libexec/frr/zebra -d -u frr -g frrvty \
    -f /etc/frr/$ns/frr.conf \
    -i /var/run/frr/$ns/zebra.pid \
    -z /var/run/frr/$ns/zebra.sock || echo "[ERROR] zebra failed in $ns"

  ip netns exec $ns env \
    VTYSH_API_SOCKET=/var/run/frr/$ns/vtysh.sock \
    /usr/libexec/frr/bgpd -d -u frr -g frrvty \
    -f /etc/frr/$ns/frr.conf \
    -i /var/run/frr/$ns/bgpd.pid \
    -z /var/run/frr/$ns/zebra.sock || echo "[ERROR] bgpd failed in $ns"
done
...
echo "[INFO] Validating BGP configuration..."
for ns in r1 r2 r3; do
  echo "--- BGP Summary for $ns ---"
  ip netns exec $ns env \
    VTYSH_API_SOCKET=/var/run/frr/$ns/vtysh.sock \
    VTYSH_CONFIG_DIR=/etc/frr/$ns \
    vtysh -c "show ip bgp summary" || echo "[WARN] $ns: vtysh failed"
done
...
echo "[INFO] [✅ READY] Routers are up. You can run commands like:"
echo "  ip netns exec r1 env VTYSH_API_SOCKET=/var/run/frr/r1/vtysh.sock VTYSH_CONFIG_DIR=/etc/frr/r1 vtysh"
echo "  ip netns exec r2 env VTYSH_API_SOCKET=/var/run/frr/r2/vtysh.sock VTYSH_CONFIG_DIR=/etc/frr/r2 vtysh"
echo "  ip netns exec r3 env VTYSH_API_SOCKET=/var/run/frr/r3/vtysh.sock VTYSH_CONFIG_DIR=/etc/frr/r3 vtysh"
