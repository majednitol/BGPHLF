...
# Start Routinator in the background (RPKI Validator)
echo "Starting Routinator..."
routinator server --rtr 127.0.0.1:3323 --http 0.0.0.0:8282 --allow-unsupported &
sleep 5  # Wait for Routinator to initialize

# Start all GoBGP routers
for i in {1..17}; do
  port=$((50050 + i))
  echo "Starting router$i..."
  gobgpd -f /gobgp/router${i}.conf --api-hosts 0.0.0.0:$port --log-level debug > /dev/stdout 2>&1 &
done

echo "Waiting for all routers to start..."
sleep 2
echo "Routers and Routinator running with debug logs:"
echo "-----------------------------------------"
